...
To invoke the configured logout handlers for the least recently used session(s), use javadoc:org.springframework.security.web.server.authentication.LogoutServerMaximumSessionsExceededHandler[] with an absolute URI for a trusted application logout endpoint.
This implementation performs an internal `POST` with the session cookie, CSRF token, and cookies that were captured when the session was registered.
When the logout handler is selected, the `XSRF-TOKEN` cookie is captured by default; use `sessionInformationCookieNames` in the concurrent sessions DSL to replace the list when different cookies are required by your logout handlers.
